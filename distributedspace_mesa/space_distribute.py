...
class Space_Distribute():
            ...
            while model.message.left_finish == False or model.message.right_finish == False: 
                ...
                for i in range(self.boundary_pass):
                    ...
                    model.message.send_left_complete == False: 
                        ...
                        model.message.process_left_neighbor(self.step_finish, self.resolver,
                                                    self.resolver_args)
    
                        model.message.process_right_neighbor(self.step_finish, self.resolver,
                                                    self.resolver_args)
    ...
    def split_model(self, x_splits, split_grid, new_mod, mod_pos):
        ...
        new_mod.message = Message(mod_pos, self.ncpus, new_mod, \
                                  self.step_finish, \
                                  self.step_args, self.resolver, \
                                  self.resolver_args, self.buffer)
...
class Message:
        ...
        def __init__(self, mod_pos, num_processes, model, step_finish,\
                     step_args, resolver, res_args, buffer):
        ...
        def process_left_neighbor(self, step_finish, resolver, resolver_args):
        ...
        def process_right_neighbor(self, step_finish, resolver, resolver_args):

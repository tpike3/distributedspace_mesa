...
class Grid:
    ...
    __slots__ = ['height', 'width', "torus", "split_pts", 'buffer', \
                     'mod_pos', 'grid', 'bus_to_left', 'bus_to_right', \
                     'limbo_agents_left', 'limbo_agents_right', 'empties']
    ...
    def __init__(self, width, height, torus):
        ...
        self.width = width
        self.torus = torus
        #attributes added doing distribution process
        self.split_pts = None
        self.buffer = None
        self.mod_pos = None

        self.grid = []
        self.bus_to_left = dict()
        self.bus_to_right = dict()
        self.limbo_agents_left = []
        self.limbo_agents_right = []
    ...
    def iter_neighborhood(self, pos, moore,
        ...
        for dy in range(-radius, radius + 1):
            for dx in range(-radius, radius + 1):
                ...
                #resimulate whole grid
                if not self.torus and \
                    (not (0 <= (dx + x + self.split_pts[self.mod_pos]) < self.split_pts[-1]) \
                    or not (0 <= dy + y < self.height)):
                        continue

                #find where they are on actual grid
                px, py, wrap = self.torus_adj((self.split_pts[self.mod_pos]+ x + dx, y + dy))
                                
                # Skip if new coords out of bounds.
                if self.out_of_bounds((px, py)):
                    continue
                
                #reconvert back to local grid with buffer
                if wrap == False: 
                    px = px - self.split_pts[self.mod_pos]
                else: 
                    px = x + dx
    ...
    def torus_adj(self, pos):
        ...
        #identify if position wrapped
        wrap = False
        if not self.out_of_bounds(pos):
            wrap = False
            return pos[0], pos[1], wrap
        elif not self.torus:
            raise Exception("Point out of bounds, and space non-toroidal.")
        else:
            #Only Y is being wrapped
            if (pos[1] >= self.height or pos[1] < 0) and \
            (pos[0] < self.width and pos[0] > 0): 
                wrap = False
            #X is being wrapped
            else: 
                wrap = True
            x, y = pos[0] % self.split_pts[-1], pos[1] % self.height
    ...
    def out_of_bounds(self, pos):
        ...
        """
        x, y = pos
        return x < 0 or x > self.split_pts[-1] or y < 0 or y >= self.height
        ...
        for x,y in cell_list: 
            ...
            if x < 0: 
                try: 
                    ...
                    in_cell.append(self.buffer['left'][x][y])
                except: 
                    
                    raise ValueError ("Your agent vision exceeds your buffer")
                    
                    
            elif x > self.width-1: 
                try: 
                    self.is_cell_empty((x,y))
                    in_cell.append(self.buffer['right'][x-self.width][y])
                except: 
    ...
    def move_agent(self, agent, pos):
        ...
        #TODO: Deal with uncertainty of user need and race conditions
        
        #move issue
        if pos[0] >= 0 and pos[0] <= self.width-1:         
            self._place_agent(pos, agent)
            agent.pos = pos
            return
        elif pos[0] < 0:
            #place in message passing
            self.bus_to_left[agent.unique_id] = (agent, pos)
            self.buffer["left"][pos[0]][pos[1]].add(agent)
            #agent.history.append(("bus_to_left", self.mod_pos, agent.model.step_num))
        ...
        #move issue
            ...
            #agent.history.append(("bus_to_right", self.mod_pos, agent.model.step_num))
            self.bus_to_right[agent.unique_id] = (agent,pos)
            self.buffer["right"][pos[0]-self.width][pos[1]].add(agent)
            #remove from schedule
            agent.model.schedule.remove(agent)
            delattr(agent, 'model')
            return "boundary"
        else:
            warnings.warn("Issue with agent position-- see move agent function")
    ...
    #added function     
    def agent_died(self, agent, schedule): 
        
        if agent.unique_id in self.bus_to_left.keys():
            del self.bus_to_left[agent.unique_id]
        elif agent.unique_id in self.bus_to_right.keys():
            del self.bus_to_right[agent.unique_id]
        else: 
            pos = agent.pos
            self._remove_agent(pos, agent)
            agent.pos = None
            # remove from schedule/ only happens if not on bus
            schedule.remove(agent)
        
        # remove if they were in the bus to pass
    ...
    def is_cell_empty(self, pos):
        ...
        if  x < 0: 
            #print ("left",x, len(self.buffer['left']))
            return True if self.buffer["left"][x][y] == self.default_val() else False
        #move issue
        elif x > self.width-1: 
            #print ("right", x,x-self.width, len(self.buffer['right']))
            return True if self.buffer["right"][x-self.width][y] == self.default_val() else False
        else: 
    ...
    def find_empty(self):
        ...
        warnings.warn(("`find_empty` is being phased out since it uses the global "
    ...
    def iter_cell_list_contents(self, cell_list):
        ...
        for x,y in cell_list: 
            ...
            if x < 0: 
                try: 
                    self.is_cell_empty((x,y))
                    in_cell.extend(self.buffer['left'][x][y])
                except: 
            ...
            #move issue
                ...
                try: 
                    self.is_cell_empty((x,y))
                    in_cell.extend(self.buffer['right'][x-self.width][y])
                except: 
...
'''
NOT ADJUSTED YET FOR DISTRIBUTION
